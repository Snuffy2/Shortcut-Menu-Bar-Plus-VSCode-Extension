...
describe('applyButtonManifest', () => {
  ...
  it('removes only the managed visibility predicate when the when clause has additional predicates', () => {
    const commands = allCommands();
    const editorTitle = allEditorMenus().map((menu) =>
      menu.command === 'ShortcutMenuBarPlus.save'
        ? {
            ...menu,
            when: '!isInDiffEditor && config.ShortcutMenuBarPlus.save && !markdownPreviewFocus',
          }
        : menu
    );
    mockFs.readFileSync.mockReturnValue(makePkg({ commands, editorTitle }));
    ...
    const written = JSON.parse((mockFs.writeFileSync as jest.Mock).mock.calls[0][1] as string);
    const saveMenu = written.contributes.menus['editor/title'].find(
      (menu: { command: string }) => menu.command === 'ShortcutMenuBarPlus.save'
    );

    expect(saveMenu.when).toBe('!isInDiffEditor && !markdownPreviewFocus');
  });

  it('removes the when property entirely when when clause consists only of the managed visibility predicate', () => {
    const commands = allCommands();
    const editorTitle = allEditorMenus().map((menu) =>
      menu.command === 'ShortcutMenuBarPlus.save'
        ? { ...menu, when: 'config.ShortcutMenuBarPlus.save' }
        : menu
    );
    mockFs.readFileSync.mockReturnValue(makePkg({ commands, editorTitle }));
    mockFs.writeFileSync.mockImplementation(() => undefined);
    ...
    expect(saveMenu).not.toHaveProperty('when');
  });

  it('deletes the when property when restoring a menu that had no original when clause', () => {
    const INTERNAL_ORIGINAL_WHEN_KEY = 'shortcutMenuBarPlusOriginalWhen';
    const MISSING_ORIGINAL_WHEN = '__shortcutMenuBarPlusMissingWhen__';
    const commands = allCommands();
    const editorTitle = allEditorMenus().map((menu) =>
      menu.command === 'ShortcutMenuBarPlus.navigateBack'
        ? {
            ...menu,
            when: 'false',
            [INTERNAL_ORIGINAL_WHEN_KEY]: MISSING_ORIGINAL_WHEN,
          }
        : menu
    );
    mockFs.readFileSync.mockReturnValue(makePkg({ commands, editorTitle }));
    ...
    const entries: ButtonEntry[] = [
      ...allBuiltinEntries(false).map((entry) =>
        entry.id === 'navigateBack' ? { ...entry, enabled: true } : entry
      ),
      ...userEntriesFromOverrides(),
    ];
    ...
    const written = JSON.parse((mockFs.writeFileSync as jest.Mock).mock.calls[0][1] as string);
    const navMenu = written.contributes.menus['editor/title'].find(
      (menu: { command: string }) => menu.command === 'ShortcutMenuBarPlus.navigateBack'
    );

    expect(navMenu).not.toHaveProperty('when');
    expect(navMenu).not.toHaveProperty(INTERNAL_ORIGINAL_WHEN_KEY);
  });

  it('stores MISSING_ORIGINAL_WHEN marker when a menu has no when property and is disabled', () => {
    const INTERNAL_ORIGINAL_WHEN_KEY = 'shortcutMenuBarPlusOriginalWhen';
    const MISSING_ORIGINAL_WHEN = '__shortcutMenuBarPlusMissingWhen__';
    const commands = allCommands();
    const editorTitle = allEditorMenus().map((menu) => {
      if (menu.command !== 'ShortcutMenuBarPlus.navigateBack') {
        return menu;
      }
      const { when: _when, ...rest } = menu;
      return rest;
    });
    mockFs.readFileSync.mockReturnValue(makePkg({ commands, editorTitle }));
    mockFs.writeFileSync.mockImplementation(() => undefined);
    ...
    expect(navMenu.when).toBe('false');
    expect(navMenu[INTERNAL_ORIGINAL_WHEN_KEY]).toBe(MISSING_ORIGINAL_WHEN);
  });

  it('returns false without writing when renameSync fails', () => {
    mockFs.readFileSync.mockReturnValue(
      makePkg({
        commands: allCommands(),
        editorTitle: allEditorMenus(),
