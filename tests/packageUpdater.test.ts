...
describe('applyUserButtonName', () => {
  ...
  it('returns false and logs error when reading package.json fails', () => {
  ...
  it('falls back to numeric default title for an unknown button index', () => {
    (mockFs.readFileSync as jest.Mock).mockReturnValue(
      JSON.stringify({
        contributes: {
          commands: [
            { command: 'ShortcutMenuBarPlus.userButton11', title: 'old title' },
          ],
        },
      })
    );

    applyUserButtonName('11', null, extensionPath);

    const written = JSON.parse(
      (mockFs.writeFileSync as jest.Mock).mock.calls[0][1] as string
    );
    expect(written.contributes.commands[0].title).toBe('user action 11');
  });

  it('returns true without writing when resetting button 10 to its already-current default', () => {
    (mockFs.readFileSync as jest.Mock).mockReturnValue(makePkg('user action 1', 'user action 0'));

    const result = applyUserButtonName('10', null, extensionPath);
